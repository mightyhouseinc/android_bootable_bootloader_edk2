...
function calls leading up to the error, in the order they occurred.</p>'''
    ...
    for frame, file, lnum, func, lines, index in records:
        ...
        call = ''
        if func != '?':
            call = f"in {strong(func)}{inspect.formatargvalues(args, varargs, varkw, locals, formatvalue=lambda value: f'={pydoc.html.repr(value)}')}"
...
'''
    ...
    for frame, file, lnum, func, lines, index in records:
        ...
        call = ''
        if func != '?':
            call = f"in {func}{inspect.formatargvalues(args, varargs, varkw, locals, formatvalue=lambda value: f'={pydoc.text.repr(value)}')}"
    ...
    def handle(self, info=None):
        ...
        if self.logdir is not None:
            ...
            try:
                ...
                file.write(doc)
                file.close()
                msg = f'<p> {path} contains the description of this error.'
            except:
                msg = f'<p> Tried to save traceback to {path}, but failed.'
            self.file.write(msg + '\n')
        try:
            self.file.flush()
        except: pass
